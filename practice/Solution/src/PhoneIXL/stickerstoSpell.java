...
public class stickerstoSpell {
	public int minStickers(String[] stickers , String target){
        ...
        for (int i = 0; i < m; i++) 
            for (char c:stickers[i].toCharArray()) stickermap[i][c-'a']++;//record different character in each sticker
        map.put("", 0);
    ...
    private int helper(Map<String, Integer> map, int[][] stickermap, String target) {
        ...
        int[] tar = new int[26];
        for (char c:target.toCharArray()) tar[c-'a']++;//record the character in the target string
        // try every sticker
        for (int i = 0; i < n; i++) {
            // optimization
            if (stickermap[i][target.charAt(0)-'a'] == 0) continue;
            StringBuilder sb = new StringBuilder();//to record the string we can not solve by this sticker
