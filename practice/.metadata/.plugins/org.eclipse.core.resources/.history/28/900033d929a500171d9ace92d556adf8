...
public class HackingTime{
	static String getKey(String tweets, String signature) {
		...
		for (int i = 1; i < short_key.length(); i++) {
			...
			if (new_tweets.substring(new_tweets.length() - signature.length())
					.equals(signature)) {
				return tmp_key;
			}
		}
	...
	static String shortenKey(String keys) {
		...
		while (end < reverse_keys.length()) {
			...
			while (tmp_begin + tmp_len <= reverse_keys.length()) {
				if (!tmp_key.equals(reverse_keys.substring(tmp_begin, tmp_begin
						+ tmp_len))) {
					break;
				} else {
					tmp_begin += tmp_len;
			...
			if (tmp_begin + tmp_len <= reverse_keys.length()) {
				...
				int i = 0;
				while (tmp_begin < reverse_keys.length()
						&& reverse_keys.charAt(tmp_begin) == tmp_key.charAt(i)) {
					i++;
					tmp_begin++;
				}
